...
/* =========================
   CEK LULUS
========================= */
if($pendaftaran['hasil_seleksi'] != 'lulus'){
    die("Kamu belum lulus seleksi!");
}
...
if(!$du){
    die("Silakan isi daftar ulang dulu!");
}

$id_du = $du['id_daftar_ulang'];

/* =========================
   CEK PEMBAYARAN
========================= */
$bayar = mysqli_fetch_assoc(mysqli_query($conn,"
SELECT * FROM pembayaran WHERE id_daftar_ulang='$id_du'
"));
...
if(isset($_POST['bayar'])){
    ...
    $newname = time().'_bayar_'.$bukti;

    move_uploaded_file($tmp,"../uploads/".$newname);

    mysqli_query($conn,"
        INSERT INTO pembayaran
        (id_pendaftaran,id_daftar_ulang,tanggal_bayar,jumlah,bukti_pembayaran,status_verifikasi)
        VALUES
        ('$id_pendaftaran','$id_du','$tanggal','$jumlah','$newname','pending')
    ");
        ...
        <div class="card-body">
            ...
            <?php } else { ?>
                ...
                <form method="POST" enctype="multipart/form-data">
                    ...
                    <input type="number" name="jumlah" class="form-control mb-2" placeholder="Jumlah Bayar" required>

                    <input type="file" name="bukti" class="form-control mb-2" required>

                    <button class="btn btn-success" name="bayar">
                        Bayar Sekarang
                    </button>
